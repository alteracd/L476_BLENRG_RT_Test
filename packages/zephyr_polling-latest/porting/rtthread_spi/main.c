...
int bt_init_hci_driver(void)
{
    ...
    HCI_TL_SPI_Init(NULL); // RT SPI ��ʼ��
    HCI_HAL_SPI_Init(NULL); //hal��SPI��ʼ��
    HCI_TL_SPI_Reset(); //�ܽ� reset

    // send_cmd(0x03, 0x003, 0, 0x00); //reset ����

    // uint16_t size = 16;
    // uint8_t dataBuff[16];
    // for (int i = 0; i < 4; ++i) {
    //     int dataLen = HCI_TL_SPI_Receive(dataBuff, size); // ����
